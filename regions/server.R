...
shinyServer(function(input, output, session) {
  ...
  output$stat_residence <- renderUI({
    ...
    if (input$colonne_residence != "rvn_trv") {
      symbole <- "%"
    } else {
      symbole <- " €"
    }

    stat_france <- paste0("France : ", db_region[ligne_fr, col], symbole)
    stat_drom <- paste0("(", paste0("Dont DROM : ", db_region[ligne_drom, col], symbole), ")")
